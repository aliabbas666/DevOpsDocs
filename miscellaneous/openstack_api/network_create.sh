...
export BLUE_VM_ID=$(curl -s -L -g -X POST 'http://'"$IP"':8774/v2.1/servers' -H 'X-Auth-Token: '"$TOKEN"'' -H 'Content-Type: application/json' --data-raw '{
    "server" : {
        "name" : "VM1",
        "imageRef" : "'"$IMG_ID"'",
        "flavorRef" : "'"$FLAVOR_ID"'",
...
export RED_VM_ID=$(curl -s -L -g -X POST 'http://'"$IP"':8774/v2.1/servers' -H 'X-Auth-Token: '"$TOKEN"'' -H 'Content-Type: application/json' --data-raw '{
    "server" : {
        "name" : "VM2",
        "imageRef" : "'"$IMG_ID"'",
        "flavorRef" : "'"$FLAVOR_ID"'",
...
export PUBLIC_VM_ID=$(curl -s -L -g -X POST 'http://'"$IP"':8774/v2.1/servers' -H 'X-Auth-Token: '"$TOKEN"'' -H 'Content-Type: application/json' --data-raw '{
    "server" : {
        "name" : "VM3",
        "imageRef" : "'"$IMG_ID"'",
        "flavorRef" : "'"$FLAVOR_ID"'",
